...
namespace
{
    ...
    protected:
        void SetUp() override
        {
            app_arg0_storage_ = "trip_qt_client_tests";
            app_argv_ = {app_arg0_storage_.data(), nullptr};
            app_ = std::make_unique<QCoreApplication>(app_argc_, app_argv_.data());
            client_ = std::make_unique<trip::QtTripClient>();
        ...
        std::unique_ptr<trip::QtTripClient> client_;
        std::unique_ptr<trip::TripHttpServer> server_;
        int app_argc_ = 1;
        std::array<char *, 2> app_argv_{};
        std::string app_arg0_storage_;
...
TEST_F(QtTripClientFixture, SupportsAuthTripListingAndRichSnapshot)
{
    ...
    const auto add_day = client().addDay(owner.token, owner.trip_id, revision, QStringLiteral("Arrival"));
    ...
    const auto add_task = client().addTask(owner.token, owner.trip_id, revision, QStringLiteral("Book hotel"));
    ASSERT_TRUE(add_task.ok);

    const auto list = client().listTrips(owner.token);
    ASSERT_TRUE(list.ok);
    ...
    const auto snapshot = client().getSnapshot(owner.token, owner.trip_id);
...
TEST_F(QtTripClientFixture, SupportsInviteBudgetChatSearchAndExportImportFlows)
{
    ...
    const auto editor_reg = client().registerUser(editor_login, QStringLiteral("pass"));
    ASSERT_TRUE(editor_reg.ok);
    const QString editor_user_id = editor_reg.payload.value(QStringLiteral("user_id")).toString();

    const auto editor_login_result = client().login(editor_login, QStringLiteral("pass"));
    ...
    const auto invite = client().createInvite(owner.token, owner.trip_id, QStringLiteral("Editor"));
    ...
    const auto accept = client().acceptInvite(editor_token, invite_code);
    ASSERT_TRUE(accept.ok);

    const auto editor_trips = client().listTrips(editor_token);
    ...
    quint64 revision = currentRevision(owner.token, owner.trip_id);
    const auto add_day = client().addDay(owner.token, owner.trip_id, revision, QStringLiteral("Museums"));
    ASSERT_TRUE(add_day.ok);
    const QString day_id = add_day.payload.value(QStringLiteral("day_id")).toString();

    revision = currentRevision(editor_token, owner.trip_id);
    const auto add_item = client().addPlanItem(
        editor_token,
        owner.trip_id,
        day_id,
        revision,
    ...
    revision = currentRevision(owner.token, owner.trip_id);
    const auto budget = client().setBudgetSettings(owner.token, owner.trip_id, revision, QStringLiteral("EUR"), 1200.0);
    ASSERT_TRUE(budget.ok);

    revision = currentRevision(editor_token, owner.trip_id);
    const auto expense = client().addExpense(
        editor_token,
        owner.trip_id,
        revision,
        55.5,
    ...
    const auto message = client().sendChatMessage(editor_token, owner.trip_id, QStringLiteral("All booked"));
    ASSERT_TRUE(message.ok);

    const auto chat = client().listChatMessages(owner.token, owner.trip_id);
    ASSERT_TRUE(chat.ok);
    ASSERT_EQ(chat.payload.value(QStringLiteral("messages_count")).toInt(), 1);

    const auto search = client().searchInTrip(owner.token, owner.trip_id, QStringLiteral("museum"));
    ASSERT_TRUE(search.ok);
    EXPECT_GE(search.payload.value(QStringLiteral("hits_count")).toInt(), 2);

    const auto summary = client().getBudgetSummary(owner.token, owner.trip_id);
    ...
    const auto events = client().getEventsSince(owner.token, owner.trip_id, 0);
    ASSERT_TRUE(events.ok);
    EXPECT_GT(events.payload.value(QStringLiteral("events")).toArray().size(), 0);

    const auto exported = client().exportTripJson(owner.token, owner.trip_id);
    ASSERT_TRUE(exported.ok);
    const QString trip_json = exported.payload.value(QStringLiteral("trip_json")).toString();
    ASSERT_FALSE(trip_json.isEmpty());

    const auto imported = client().importTripJson(owner.token, trip_json);
    ASSERT_TRUE(imported.ok);
    ASSERT_FALSE(imported.payload.value(QStringLiteral("trip_id")).toString().isEmpty());

    const auto role_change = client().changeMemberRole(owner.token, owner.trip_id, editor_user_id, QStringLiteral("Viewer"));
    ASSERT_TRUE(role_change.ok);

    const auto remove_member = client().removeMember(owner.token, owner.trip_id, editor_user_id);
...
TEST_F(QtTripClientFixture, ReceivesRealtimeUpdatesAndReconnectBacklog)
{
    ...
    live_socket.open(client().updatesWebSocketRequest(owner.token, owner.trip_id, base_revision));
    ASSERT_TRUE(waitForConnected(live_socket));

    const auto add_live = client().addTask(owner.token, owner.trip_id, base_revision, QStringLiteral("Realtime task"));
    ...
    const auto add_backlog = client().addTask(owner.token, owner.trip_id, after_first, QStringLiteral("Backlog task"));
    ...
    reconnect_socket.open(client().updatesWebSocketRequest(owner.token, owner.trip_id, after_first));
